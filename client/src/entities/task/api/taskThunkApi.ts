...
>(TASK_THUNKS_TYPES.CREATE, async (newTaskData, { rejectWithValue }) => {
  try {
    const { data } = await axiosInstance.post<
      ApiResponseSuccess<Task>
    >(TASKS_API_ROUTES.ALL_TASKS, newTaskData);
    return data
  } catch (error) {
    const err = error as AxiosError<ApiResponseReject>;
    return rejectWithValue(err.response!.data);
...
export const updateByIdThunk = createAsyncThunk<
  ApiResponseSuccess<Task>,
  { id: number; updatedTask: AxiosRequestConfig<RawTaskData> | undefined },
  { rejectValue: ApiResponseReject }
>(TASK_THUNKS_TYPES.UPDATE, async ({id, updatedTask}, { rejectWithValue }) => {
  try {
    const { data } = await axiosInstance.delete<
      ApiResponseSuccess<Task>
    >(TASKS_API_ROUTES.ALL_TASKS+`/${id}`, updatedTask);
    return data
  } catch (error) {
    const err = error as AxiosError<ApiResponseReject>;
    return rejectWithValue(err.response!.data);
